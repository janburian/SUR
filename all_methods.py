...
def retezovaMapa(maticeVzdalenosti, index_pocatek):
    ...
    for i in range(len(listMinVzdalenosti) - 1):
        if ((listMinVzdalenosti[i] - listMinVzdalenosti[
            i + 1]) > dostatecnaVzdalenost):  # dostatecny rozdil mezi vzdalenostmi
            pocetTrid += 1
...
def iterativniOptimalizace(listTrid, stredy, data):
    for i in range(len(data)):
        ...
        for j in range(len(stredy)):
            ...
            if data[i] in trida:
                A = (len(listTrid[pozicePrvku]) / ((len(
                    listTrid[pozicePrvku])) - 1)) * vzdalenost  # výpočet vzdálenosti, pokud prvek je v dané třídě
                listA.append(A)
                pomocna = A
            else:
...
def knn(rozdeleni_z_kmeans, vektor_klasifikace, K):
    ...
    if (K == 1):
        ...
        indexTridyMin = listMinDist.index(
            finalMinDist)  # zjistim, tridu, ve ktere je nejblizsi bod, cili kam budu klasifikovat bod (reseni pro 1 nejblizsiho souseda)
...
def Rosenblatt_pomocna(listVektoru, pomocnyListOmega, q):
    for idx_Ros in range(5000):  # zamezeni nekonecneho cyklu
...
def konstantniPrirustky_pomocna(listVektoru, pomocnyListOmega, q):
    for idx_konstPri in range(5000):  # zamezeni nekonecneho cyklu
        kontrola = kontrolaOK(listVektoru, pomocnyListOmega, q)
        if (jsouVsechnyOk(kontrola)):
...
def upraveneKonstantniPrirustky_pomocna(listVektoru, pomocnyListOmega, q):
    for idx_konstPriUpr in range(5000):
        kontrola = kontrolaOK(listVektoru, pomocnyListOmega, q)
        if (jsouVsechnyOk(kontrola)):
        ...
        for i in range(len(listVektoru)):
            ...
            while (soucin_Q_vektorRozsireny < 0):
                q = vypoctiNoveQ(q, obrazovyVektor_rozsireny, pomocnyListOmega[i], 0.1)
                mamNoveQ = True
                soucin_Q_vektorRozsireny = vypoctiSoucinQ_vektorRozsireny(q, obrazovyVektor_rozsireny,
                                                                          pomocnyListOmega[i])
...
def vykresliBayes(stredy_z_k_means, rozdeleniDoTrid_k_means, list_apr_ppst_tridy, list_kovariancniMatice, data):
    ...
    for ix in np.arange(X_min_max[0], X_min_max[1], 1):
        for jy in np.arange(Y_min_max[0], Y_min_max[1], 1):
            bod = (ix, jy)
            indexTridy = Bayesovsky_klasifikator(stredy_z_k_means, rozdeleniDoTrid_k_means, list_apr_ppst_tridy,
                                                 list_kovariancniMatice, data, bod)
...
def vykresliUpravenouMetoduKonstPrirustku(rozdeleniDoTrid_k_means, usecky_konstPrir_uprav, listQ_konstPrir_uprav,
                                          segmentyTrid, data):
...
if __name__ == "__main__":
    ...
    usecka1 = ZjistiDeliciPrimkuRosenblatt(rozdeleniDoTrid_k_means[0],
                                           SpojTridy(rozdeleniDoTrid_k_means[1], rozdeleniDoTrid_k_means[2]), q0)
    usecky_Ros.append(usecka1[0])
    listQ_Ros.append(usecka1[1])

    usecka2 = ZjistiDeliciPrimkuRosenblatt(rozdeleniDoTrid_k_means[1],
                                           SpojTridy(rozdeleniDoTrid_k_means[0], rozdeleniDoTrid_k_means[2]), q0)
    usecky_Ros.append(usecka2[0])
    listQ_Ros.append(usecka2[1])

    usecka3 = ZjistiDeliciPrimkuRosenblatt(rozdeleniDoTrid_k_means[2],
                                           SpojTridy(rozdeleniDoTrid_k_means[0], rozdeleniDoTrid_k_means[1]), q0)
    ...
    usecka1 = ZjistiDeliciPrimkuKonstPrirustky(rozdeleniDoTrid_k_means[0],
                                               SpojTridy(rozdeleniDoTrid_k_means[1], rozdeleniDoTrid_k_means[2]), q0)
    usecky_KonstPrir.append(usecka1[0])
    listQ_KonstPrir.append(usecka1[1])

    usecka2 = ZjistiDeliciPrimkuKonstPrirustky(rozdeleniDoTrid_k_means[1],
                                               SpojTridy(rozdeleniDoTrid_k_means[0], rozdeleniDoTrid_k_means[2]), q0)
    usecky_KonstPrir.append(usecka2[0])
    listQ_KonstPrir.append(usecka2[1])

    usecka3 = ZjistiDeliciPrimkuKonstPrirustky(rozdeleniDoTrid_k_means[2],
                                               SpojTridy(rozdeleniDoTrid_k_means[0], rozdeleniDoTrid_k_means[1]), q0)
    ...
    usecka1 = ZjistiDeliciPrimkuKonstPrirustky_uprav(rozdeleniDoTrid_k_means[0],
                                                     SpojTridy(rozdeleniDoTrid_k_means[1], rozdeleniDoTrid_k_means[2]),
                                                     q0)
    usecky_konstPrir_uprav.append(usecka1[0])
    listQ_konstPrir_uprav.append(usecka1[1])

    usecka2 = ZjistiDeliciPrimkuKonstPrirustky_uprav(rozdeleniDoTrid_k_means[1],
                                                     SpojTridy(rozdeleniDoTrid_k_means[0], rozdeleniDoTrid_k_means[2]),
                                                     q0)
    usecky_konstPrir_uprav.append(usecka2[0])
    listQ_konstPrir_uprav.append(usecka2[1])

    usecka3 = ZjistiDeliciPrimkuKonstPrirustky_uprav(rozdeleniDoTrid_k_means[2],
                                                     SpojTridy(rozdeleniDoTrid_k_means[0], rozdeleniDoTrid_k_means[1]),
                                                     q0)
    ...
    vykresliUpravenouMetoduKonstPrirustku(rozdeleniDoTrid_k_means, usecky_konstPrir_uprav, listQ_konstPrir_uprav,
                                          segmentyTrid, data)
